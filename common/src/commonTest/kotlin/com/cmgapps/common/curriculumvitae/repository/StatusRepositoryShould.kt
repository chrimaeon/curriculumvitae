...
class StatusRepositoryShould {

    private lateinit var repository: StatusRepository

    @BeforeTest
    fun setup() {
        repository = StatusRepository(CvApiService(mockClient, Url(BaseUrl)))
    }

    @ExperimentalTime
    @Test
    @Ignore // https://youtrack.jetbrains.com/issue/KTOR-537
    fun get_domain_status() = runTest {
        repository.getStatus().test {
            assertEquals(StubDomainStatus(), awaitItem())
            awaitComplete()
        }
    }
}
